...
This amends a settled trade; it does not fill a gap.

FR-7.2 and FR-7.3 say nothing is dropped and nothing is capped. An age limit is a
cap, and it is taken deliberately: a durable queue can outlive the process that
filled it, so a backlog can be old in a way an in-memory one never is.
...
summary stays worth hearing and not a property of the machine, so a later
reader should leave it alone and not correct it toward a default.
