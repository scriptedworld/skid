...
`paplay` is `/usr/bin/pacat`, and the server is PulseAudio 15.0.0 on PipeWire
1.4.2.

A command line and not a name from a fixed list, so a player nobody anticipated
needs no change here to work.

`aplay` is present and talks to ALSA without following the default sink, so
configuring it would not satisfy FR-1.4. Configuring it anyway is allowed, and
