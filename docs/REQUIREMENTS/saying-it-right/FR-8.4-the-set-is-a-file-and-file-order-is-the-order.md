...
Stated when FR-7.7 admitted regular expressions and made ordering something
...
The file is YAML, and nothing about the format has to preserve order. A YAML
sequence carries its order in the decoded structure, so the list skid reads is
already in file order and the list it writes comes back in the same one.
