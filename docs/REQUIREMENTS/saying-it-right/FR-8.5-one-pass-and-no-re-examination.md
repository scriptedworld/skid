...
Position first, file order second. That pairing is the whole rule. "The earlier
entry in the file wins the overlapping region" is a different priority, and the
two disagree on ordinary input: with `b -> x` first and `ab -> Z` second,
applied to `ab`, entry priority gives `ax` and position priority gives `Z`.
