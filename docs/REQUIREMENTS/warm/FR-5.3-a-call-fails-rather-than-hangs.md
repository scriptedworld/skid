...
Stated as a property and not a list, because a list is the defect. A wording
that enumerates two cases, absent and wedged, reads as complete to everyone who
meets it, including the reviews that produced it, and the case that actually bit
was in neither:
...
- **Wedged.** Running and not answering, which nothing removes. `Type=notify`
  ...
  stop answering after both. `WatchdogSec=120` in `skid.service` restarts a
  serve loop that stops making progress.
...
The third is why the row is a property. Any enumeration invites a reader to
...
It does not literally wait forever, and the true figure is worse for this row. A
call left to run its course ended after the full half hour:
...
requirement, which is why it is stated as a property. An abort at 1800 seconds carrying no diagnosis is the failure being survived by
...
Half an hour is also indistinguishable from slow work. `TIMEOUT = 300.0` in
...
tool error naming the socket, because a client can report an error and cannot
report silence.
