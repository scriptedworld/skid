...
Nothing is cached on either side of the socket, and that is the reason for the
shape. MCP over HTTP puts a session id in the service's memory, and a restart
forgets it: the service then answers every later request `404 Session not found`
with a null id, which a JSON-RPC client cannot match to anything it asked, so it
waits until something outside gives up, measured at 1800 seconds. Holding no
session removes the failure instead of recovering from it.
...
What that want costs, measured: the warm service holds 1.66 GB, of which
1.31 GB is private and dirty and so cannot be shared with a second instance. N
stdio servers is therefore 1.31 GB per session: 12.2 GB across nine sessions
running at once, against 2.0 GB for one service and nine 35 MB shims, on a
machine with 20 GB free. Cold start is 6.2 seconds.

So the design is not forced, and the alternative is not cheap either. FR-5.1
...
- One place FR-4.4 is true, with no protocol about whose submission is
  next.
- One place a failure is visible, which FR-4.7 requires.
- No start protocol at all, because systemd owns the socket and the
  lifetime. No lock file, no stale-socket unlink and no bind-then-rename to make
  the path mean ready.
...
Stale state is systemd's problem, not skid's. A socket path outlives the process
that bound it: measured, after `SIGKILL` the path remains, a fresh bind gets
`EADDRINUSE` and a client gets `ECONNREFUSED`. Under socket
...
crash waits for the restart instead of meeting a dead path.

So there is no start protocol: no lock file, no stale-socket unlink, no
bind-then-rename, no readiness race.
...
A Linux abstract socket was considered and rejected on the same axis. Measured:
bind to `"\0skid"`, a second bind gets `EADDRINUSE` while
...
Nothing about skid is spawned by an agent, so none of the stdio hazards apply: no inherited JSON-RPC stream to corrupt with a torch
...
be handed something that has just come free instead of doubling up on a voice
...
speech and not from submission, for the reason the greeting is: what a
...
on the list as an English speaker with an Italian accent and not as an
Italian phonemiser reading English badly.

Pipelines are cached, not dropped, on a change, and share one model.
...
Measured end to end on this machine before any of skid existed:
...
YAML, read and written by wrench against a schema in `skid/schemas.py`, like
every structured file in the ecosystem.
...
not know is refused by name, not ignored, so `voce: af_bella` fails at
...
a truncated config and lose the substitution set. The read-modify-write is done under the same mtime check, so a hand edit
...
It does not preserve comments. A comment is not the only place a reason can
live: `docs/config.sample.yaml` carries the reasons, and the live file carries
values alone, so no style-preserving round trip is needed. FR-7.1 records the
retired comment clause.
...
it is reported and not silently replaced by defaults, because overwriting it
...
service and not the call (FR-5.3). Socket activation removes the absent
...
whole, not only as a run of individual failures.
...
never a truncated entry that parses to nonsense and blocks the queue.
...
interrupted, and is discarded, not replayed, because FR-4.4 makes a
...
Restart, not reboot. tmpfs, so a reboot starts silent instead of reading
...
and not spoken. Downtime counts toward it, so a service away for ten minutes
...
The declaration is not a ceiling. kokoro is reported first-hand to run on 3.13
and 3.14, and has simply not had a release since. A resolver still enforces
...
clone, which a relative path would not.

It is installed as an ordinary copy. mypy cannot follow a PEP 660 import hook, so
under an editable install wrench's `py.typed` is invisible and every import of it
is an error.

kokoro 0.9.4 and its torch stack install and run under 3.12.14, and the WAV
writer is the stdlib's.

The MCP SDK under 3.12 is settled: measured on skid's own interpreter, `mcp`
2.1.1 under 3.12.14. A reading taken under 3.14.7, this machine's default, says
only that the SDK exists, not that it is present for skid.

waitress and not gunicorn, because it serves a socket that is already
